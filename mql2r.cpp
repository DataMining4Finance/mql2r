   ...
   */
   ...
   int MT4_EXPFUNC RGetXTS(char* expression,RateInfo* rates,const int rates_total)
   {
	   try
	   {
		   ...
		   int fetch_num = std::min((size_t)rates_total,series.Opens().size());

		   for(int i=0;i<fetch_num;++i)
		   {
			   rates[i].open = series.Opens()[i];
			   rates[i].high = series.Highs()[i];
			   rates[i].low = series.Lows()[i];
			   rates[i].close = series.Closes()[i];
			   rates[i].vol = series.Volumes()[i];
			   rates[i].ctm = series.Times()[i].getFractionalTimestamp();
		   }
